...
@section('header')
<x-ui.page-header title="Klien" pretitle="Landing Page">
    <x-slot:actions>
        ...
        @can('public.cms.client.create')
            <x-ui.button type="create" class="ajax-modal-btn"
                data-url="{{ route('cms.client.create') }}"
                        ...
                        <div class="position-absolute top-0 end-0 mt-2 me-2">
                            <x-ui.dropdown class="btn btn-action text-secondary">
                                @can('public.cms.client.update')
                                    <x-ui.dropdown-item type="edit" href="javascript:void(0)" :url="route('cms.client.edit', $client)" data-modal-title="Edit Klien" />
                                @endcan
                                @can('public.cms.client.delete')
                                    <x-ui.dropdown-item type="delete" href="javascript:void(0)" :url="route('cms.client.destroy', $client)" title="Hapus Klien?" />
                                @endcan
                            </x-ui.dropdown>
                        </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    Sortable.create(grid, {
        animation: 150,
        handle: '.cursor-move',
        onEnd: () => axios.post('{{ route('cms.client.reorder') }}', {
