...
class SaveDesignRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        foreach (['sectionVariants', 'sectionColors', 'sectionSettings'] as $key) $rules[$key] = ['nullable', 'array:'.implode(',', array_keys($sections))];
        foreach ($sections as $key => $section) $rules["sectionVariants.$key"] = ['sometimes', 'string', Rule::in(array_keys($section['variants'] ?? []))];
        ...
        foreach (['bg', 'text', 'heading', 'accent', 'pretext_color', 'text_color', 'posttext_color'] as $key) $rules["sectionColors.*.$key"] = ['nullable', 'string', 'regex:/^#(?:[0-9a-fA-F]{3}|[0-9a-fA-F]{6})$/'];
        ...
        foreach (['active', 'show_login', 'show_topbar', 'showTopbar'] as $key) $rules["sectionSettings.*.$key"] = ['sometimes', 'boolean'];
        foreach (['title', 'pre_title', 'subtitle', 'topbar_hours'] as $key) $rules["sectionSettings.*.$key"] = ['nullable', 'string', 'max:1000'];
