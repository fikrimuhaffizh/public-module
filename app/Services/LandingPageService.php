...
class LandingPageService
{
    ...
    public function template(?string $previewTemplate = null): string
    {
        ...
        $published = $this->design()['template'] ?? null;
        if ($this->themes->isValid($published)) {
            return $published;
        }
    ...
    {
        abort_unless($this->themes->isValid($template), 422, 'Tema tidak valid.');
        $settings = LandingPageSetting::forCurrentTenant();
        if (($settings->design['template'] ?? null) === $template) {
            return;
        }
        $settings->design = ['template' => $template];
        $settings->save();
    }
    ...
    public function saveDesign(string $template, array $design, array $sectionSettings = []): void
    {
        if (! $this->themes->isValid($template)) {
            abort(422, 'Tema tidak valid.');
        }
        try {
            DB::transaction(function () use ($template, $design, $sectionSettings) {
                $settings = LandingPageSetting::forCurrentTenant();
                $settings->design = array_merge($design, ['template' => $template]);
                $settings->save();
                $this->saveSectionSettings($sectionSettings);
                logActivity('public', 'Updated landing page design');
            });
        } finally {
            $this->tenantConfig->clearCache(sys_tenant_id());
        }
        // Background deletion is explicit: other templates and drafts may still use it.
    }
    ...
    public function shared(string $template, bool $preview = false): array
    {
        ...
        return [
            ...
            'canCustomizeDesign' => sys_tenant_id(null) !== null && can('public.cms.settings.update'),
            'designScope' => (sys_tenant_id() ?? 'global').':'.(auth()->id() ?? 'guest'),
            'site' => [
    ...
    {
        $item = ['id' => $menu->getKey(), 'title' => $menu->title, 'url' => match ($menu->type) {
            'page' => $menu->page ? route('public.page.show', ['page' => $menu->page->slug]) : '#', 'route' => $menu->url && Route::has($menu->url) ? route($menu->url) : '#', default => $menu->url ?: '#',
        }, 'target' => $menu->target];
        if ($menu->children && $menu->children->count() > 0) {
            $item['children'] = $menu->children->filter(fn (Menu $c) => $c->is_active)->map(fn (Menu $c) => $this->mapMenuItem($c))->values();
        }
    ...
    public function reorderSections(string $area, array $ids): void
    {
        ...
        $i = 1;
        foreach ($ids as $id) {
            LandingSection::where('tenant_id', $tenantId)->where('landing_section_id', decryptId($id))->update(['sort_order' => $i++]);
        }
    }
    ...
    public function sectionData(LandingSection $section): array
    {
        ...
        switch ($section->section_key) {
            case 'hero': $data['hero'] = $this->heroContent($section);
                break;
            case 'stats': case 'statistic': $data['stats'] = Section::ofType('statistic')->where('is_active', true)->orderBy('sort_order')->limit($section->limit_data ?? 4)->get()->map(fn (Section $i) => $this->mapStatistic($i))->values();
                break;
            case 'products': case 'product': $data['products'] = Section::ofType('product')->where('is_active', true)->orderBy('sort_order')->limit($section->limit_data ?? 6)->get()->map(fn (Section $i) => $this->mapProduct($i))->values();
                break;
            case 'features': case 'feature': $data['features'] = Section::ofType('feature')->where('is_active', true)->orderBy('sort_order')->limit($section->limit_data ?? 6)->get()->map(fn (Section $i) => $this->mapFeature($i))->values();
                break;
            case 'testimonials': case 'testimonial': $data['testimonials'] = Section::ofType('testimonial')->where('is_active', true)->orderBy('sort_order')->limit($section->limit_data ?? 3)->get()->map(fn (Section $i) => $this->mapTestimonial($i))->values();
                break;
            case 'clients': case 'client': $data['clients'] = Section::ofType('client')->where('is_active', true)->orderBy('sort_order')->limit($section->limit_data ?? 8)->get()->map(fn (Section $i) => $this->mapClient($i))->values();
                break;
            case 'faq': $data['faqs'] = FAQ::where('is_active', true)->orderBy('seq')->limit($section->limit_data ?? 8)->get()->map(fn (FAQ $i) => $this->mapFaq($i))->values();
                break;
            case 'pengumuman': case 'announcement': $data['announcements'] = $this->announcements($section->limit_data ?? 6);
                break;
            case 'cta': $cta = Section::ofType('cta')->where('is_active', true)->latest('updated_at')->first();
    ...
    {
        return ['id' => $page->getKey(), 'title' => $page->title, 'excerpt' => Str::limit(strip_tags($page->content), 135), 'url' => route('public.page.show', $page->slug)];
    }
    ...
    {
        return ['id' => $slide->getKey(), 'title' => $slide->title, 'caption' => $slide->caption, 'image' => $slide->large_url, 'link' => $slide->link];
    }
    ...
    private function mapFeature(Section $item): array
    {
        return ['id' => $item->getKey(), 'title' => $item->title, 'description' => $item->description, 'icon' => $item->icon, 'image' => $item->image_url];
    }

    private function mapProduct(Section $item): array
    {
        return ['id' => $item->getKey(), 'name' => $item->title, 'slug' => $item->slug, 'shortDescription' => $item->getSetting('short_description'), 'description' => $item->description, 'image' => $item->image_url, 'demoUrl' => $item->getSetting('demo_url')];
    }

    private function mapStatistic(Section $item): array
    {
        return ['id' => $item->getKey(), 'label' => $item->title, 'value' => $item->getSetting('value'), 'icon' => $item->icon];
    }

    private function mapClient(Section $item): array
    {
        return ['id' => $item->getKey(), 'name' => $item->title, 'logo' => $item->image_url, 'website' => $item->getSetting('website')];
    }

    private function mapPricing(Pricing $item): array
    {
        return ['id' => $item->getKey(), 'name' => $item->name, 'slug' => $item->slug, 'description' => $item->description, 'price' => $item->price, 'period' => $item->period, 'features' => $item->features ?? [], 'highlight' => $item->highlight];
    }

    private function mapCta(Section $cta): array
    {
        return ['title' => $cta->title, 'description' => $cta->description, 'buttonText' => $cta->getSetting('button_text'), 'buttonLink' => $cta->getSetting('button_link'), 'backgroundImage' => $cta->image_url];
    }

    private function mapFaq(FAQ $item): array
    {
        return ['id' => $item->getKey(), 'question' => $item->question, 'answer' => $item->answer, 'category' => $item->category];
    }

    private function mapTestimonial(Section $item): array
    {
        return ['id' => $item->getKey(), 'name' => $item->title, 'position' => $item->getSetting('position'), 'organization' => $item->getSetting('organization'), 'quote' => $item->description, 'rating' => $item->getSetting('rating'), 'photo' => $item->image_url];
    }

    private function mapPartner(Section $p): array
    {
        return ['id' => $p->getKey(), 'name' => $p->title, 'category' => $p->getSetting('category'), 'url' => $p->getSetting('website_url'), 'logo' => $p->image_url];
    }
}
